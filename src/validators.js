  ...
  if (req.method === 'POST' || req.method === 'PUT') {
    if (Object.keys(req.body).length > 0) {
      ...
      if (memorySize > config.SIZE_LIMIT * 1024) {
        throwError(
          `JSON body is too large. Should be less than ${config.SIZE_LIMIT}KB`,
          413
        )
...
const fetchBoxWhitelistedBool = async boxId => {
  try {
    ...
    console.log(e)
    // benefit of network layer issues to the user
    return true
  }
}

const memFetchBoxWhitelistedBool = memoize(fetchBoxWhitelistedBool, {
  promise: true,
...
const checkBoxIdWhitelisted = async (req, res, next) => {
  try {
    if (
      ...
      try {
        const allowed = await memFetchBoxWhitelistedBool(
          req.box.includes('_') ? req.box.split('_')[1] : req.box
        )
        if (allowed) next()
